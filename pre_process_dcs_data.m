...
if isfield(dcs_file,'average_time_segments') && length(dcs_file.average_time_segments)>1
    ...
    for segment=1:size(dcs_file.average_time_segments,1)
        data_to_fit(:,:,segment)=mean(corrset_detavgd(:,:,index_segment_array(segment,1):index_segment_array(segment,2)),3);
...
elseif isfield(dcs_file,'average_time_segments') && dcs_file.average_time_segments==0
    ...
    for ch=1:size(corrset_detavgd,2)
        ...
        for tau_point=1:size(corrset_detavgd,1)
            temp_arr=squeeze(corrset_detavgd(tau_point,ch,:));
            decimate_temp_arr=decimate(temp_arr,dcs_file.decimate_factor);
            decimate_corrset(tau_point,ch,:)=decimate_temp_arr;
        end
    end
    decimate_times=decimate(temp_time_arr,dcs_file.decimate_factor);
    
    % making a moving mean
    movmean_data=movmean(decimate_corrset,dcs_file.moving_mean_window_length,3);
    ...
    last_time_seg=floor(size(movmean_data,3)/avg_data_to_fit)*avg_data_to_fit;
    time_arr(1)=0;
    idx=1;
    for I=1:avg_data_to_fit:last_time_seg
        data_to_fit(:,:,idx)=mean(movmean_data(:,:,I:(I+avg_data_to_fit-1)),3);
