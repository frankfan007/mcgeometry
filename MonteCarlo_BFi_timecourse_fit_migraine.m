...
elseif ispc
    raw_data_serverpath='\\oxm.nmr.mgh.harvard.edu';
...
working_folder_unix=['/space/blinky/4/users/hypercapnia/juliette_data/Subj005/20121106/'];
...
raw_data_basepath=[raw_data_serverpath '/space/oxm/1/users/juliette/Projects/Migraine_Phoebe/Data'];
dir_struct.dcs_file_dir=[raw_data_basepath filesep '20121106' filesep 'Subj005']; % directory where DCS data is stored
dir_struct.mr_dir=[raw_data_basepath 'MRI/fsrecons/mdh005_121108/mri/']; % directory where T1 is stored
...
dcs_file.filename='20121106_Subj005_HC1_SyncData.mat'; % EDIT
dcs_file.det_averaging={1:1,2:8}; 
...
dcs_file.moving_mean_window_length=3; 
dcs_file.avg_span=5; 
...
analytical_fit_options.lambda_dcs = 785*1e-6; % mm-1
analytical_fit_options.rhos_arr=[8 30]; % mm
...
mc_param.inp_filename='stairhead_8_30_mm_085_mus_rotate_0'; % REQUIRED; write without extension .inp ; will use input file if exists or create one under variable name
...
ref_param.det_distances=[8 30]; % mm
ref_param.fiducial_pos=[]; % if not empty, will automatically use this value as fiducial position
ref_param.rotate_deg=[]; % if not empty, will automatically use this value as rotational degree
...
concatenate_tissue_layers_array={1:4,5:13,14:22}; % EDIT
...
elseif gen_options.heat_plot
    dcs_file.average_time_segments=1;
end
...
tcd_struct=prepare_tcd_data_from_migraine_dcsfile(dcs_file,time_arr);
...
save([save_plot_fullname '.mat'],...
    'dir_struct','gen_options','tcd_struct',...
